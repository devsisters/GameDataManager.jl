...
function export_dialoguecommand()
    if Sys.iswindows()
        unitypath = "C:/Program Files/Unity/Editor/2019.3.7f1/Editor/unity.exe" 
    else 
        unitypath = "/Applications/Unity/Hub/Editor/2019.3.7f1/Unity.app/unity"
    end

    if !isfile(unitypath)
        throw(AssertionError("유니티를 찾을 수 업습니다. 경로를 확인해 주세요 / $unitypath"))
